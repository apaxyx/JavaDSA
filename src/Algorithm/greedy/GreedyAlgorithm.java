...
贪心算法（贪婪算法）
1.贪婪算法是指在对问题进行求解时，在每一步选择中都采取最好或者最优（即最有利）的选择，从而希望
能够导致结果是最好或者最优的算法
2.贪婪算法所得到的结果不一定是最优的结果（有时候会是最优解），但是都是想对近似（接近）最优解的
结果
集合覆盖问题
假设存在下面需要付费的广播台，以及广播台信号可以覆盖的地区。如何选择最少的广播台，
让所有的地区都可以接收到信号
广播台     覆盖地区
K1          "北京","上海","天津"
K2          "广州","北京","深圳"
...
思路分析：
使用贪婪算法，效率高：
目前并没有算法可以快速计算到到准备的值，使用贪心算法，则可以得到非常接近的解，并且效率高
    ...
    public static void main(String[] args) {
        ...
        while(allAreas.size() != 0){//如果allAreas不为0，则表示还没有覆盖到所有的地区
            ...
            for (String key : broadcasts.keySet()) {
                ...
                //求出tempSet和allAreas集合的交集，交集会赋纷呈tempSet
